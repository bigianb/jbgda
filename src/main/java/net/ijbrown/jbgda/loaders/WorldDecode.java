...
public class WorldDecode
{
    ...
    private void decodeWorld(String rootDir, String rootDirOrig, String lmpName, String worldName, boolean minimapOnly) throws IOException
    {
        ...
        Path outDirPath = outDirFile.toPath();

        extractMiniMap(worldName + "_minimap", outDirPath);
        if (!minimapOnly) {
            String txt;
            txt = disassemble(outDirFile, new File(rootDirOrig + lmpName + ".tex"));
            writeFile(worldName + ".world.txt", outDirPath, txt);
        }
    }

    public void decode(byte[] data, Path outDir, Path levelTexPath, String worldName) throws IOException {
        fileData = data;
        extractMiniMap(worldName + "_minimap", outDir);

        String txt;
        txt = disassemble(outDir.toFile(), levelTexPath.toFile());
        writeFile(worldName + ".world.txt", outDir, txt);
    }

    private void writeFile(String filename, Path outDirFile, String txt) throws IOException
    {
        File file = new File(outDirFile.toFile(), filename);
    ...
    private void extractMiniMap(String outputFilename, Path outDir) throws IOException
    {
    ...
    private String disassemble(File outDirFile, File levelTexFile)
    {
        ...
        sb.append("topo start col: ").append((DataUtil.getLEInt(fileData, 0x08))).append("\r\n");
        sb.append("topo start row: ").append((DataUtil.getLEInt(fileData, 0x0C))).append("\r\n");
        ...
        sb.append("num topo cols: ").append(cols).append("\r\n");
        sb.append("num topo rows: ").append(rows).append("\r\n");
        ...
        int numTopoElements = DataUtil.getLEInt(fileData, 0x1C);
        sb.append("num topo elements: ").append(HexUtil.formatHex(numTopoElements)).append("\r\n");


        int topoElementsOffset = DataUtil.getLEInt(fileData, 0x20);
        sb.append("topo elements offset: ").append(HexUtil.formatHex(topoElementsOffset)).append("\r\n");

        sb.append("\r\n");

        int polysets = DataUtil.getLEInt(fileData, 0x24);
        sb.append("Polyset Base (world.24): ").append(HexUtil.formatHex(polysets)).append("\r\n");
        ...
        int doors = DataUtil.getLEInt(fileData, 0x54);
        sb.append("Doors offset: ").append(HexUtil.formatHex(doors)).append("\r\n");
